...
public class PrintAnyOneSubsequenceWithTargetSum<T> {
    ...
    public static void main(String[] args) {
        ...
        PrintAnyOneSubsequenceWithTargetSum<Integer> object = new PrintAnyOneSubsequenceWithTargetSum<>();
    ...
    private boolean generateSubsequences(List<T> input, int index, List<T> current, List<List<T>> output, int targetSum) {
        ...
        current.add(element);
        boolean result1 = generateSubsequences(input, index + 1, current, output, targetSum - (Integer) element);
        if (result1) return true;
        // case where the element is not taken, then targetSum remains same
        current.remove(element);
        boolean result2 = generateSubsequences(input, index + 1, current, output, targetSum);
